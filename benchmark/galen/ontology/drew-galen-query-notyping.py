...
os.system(("rm -f logs/%s-notyping-%s.txt")%(benchmark, T))
...
for num in xrange(10, 51, 10):
#        fact = "df-%s.user-%d.dl" % (benchmark, num)
#	print fact
        cmd = "{0} -brave -OMS- df-galen-query-notyping.dl el.dl el-i.dl el-i-n.dl el-galen.dl df-galen.fact-{1}.dl df-query.dlv".format(DLV, num)
...
#        exit(0)
        ...
        cmd = "{0} -brave -OMS- df-galen-query-notyping.dl el.dl el-i.dl el-i-n.opt.dl el-galen.dl df-galen.fact-{1}.dl df-query.dlv".format(DLV, num)
        (e2,t2)=bm.timelimit(cmd, 300)
	log.write(cmd + "\n")
	print "dlv ", t2,"s"

        cmd = "{0} df-galen-query-notyping.dl el.dl el-i.dl el-i-n.dl el-galen.dl df-galen.fact-{1}.dl df-filter.clingo".format(CLINGO, num)
        (e3,t3)=bm.timelimit(cmd, 300)
	log.write(cmd + "\n")
	print "clingo ", t3,"s"

        cmd = "{0} df-galen-query-notyping.dl el.dl el-i.dl el-i-n.opt.dl el-galen.dl df-galen.fact-{1}.dl df-filter.clingo".format(CLINGO, num)
